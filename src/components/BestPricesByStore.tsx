...
const BestPricesByStore: React.FC<BestPricesByStoreProps> = ({ comparisonData }) => {
  ...
  const getBestPricesByStore = (): StoreWithBestPrices[] => {
    ...
    const bestPriceMap: Map<string, { storeId: string; price: number }> = new Map();
    ...
    products.forEach((product) => {
      ...
      if (bestStoreId) {
        bestPriceMap.set(product.id, { storeId: bestStoreId, price: bestPrice });
      }
    });
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      <h2 className="text-xl font-semibold mb-6">Itens Mais Baratos por Mercado</h2>
              ...
              <CardHeader className="pb-2 bg-gray-50">
                <CardTitle className="flex justify-between items-center">
                  ...
                  <span className="text-app-green">
                    {items.length} {items.length === 1 ? "item" : "itens"} mais barato{items.length === 1 ? "" : "s"}
                  </span>
                </CardTitle>
              </CardHeader>
