...
export const AlertsManager: React.FC<{ className?: string }> = ({ className }) => {
    ...
    try {
      ...
      if (selectedAlert && selectedAlert.id) {
        ...
        toast({
          ...
          description: "Alerta foi atualizado com sucesso"
        });
      } else {
        await alertsService.createAlert(user.id, formData as any);
        toast({
          title: "Alerta criado",
          description: "Alerta foi criado com sucesso"
        });
      }
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <Tabs defaultValue="list" className="space-y-4">
        ...
        <TabsContent value="list" className="space-y-4">
          {alerts.map((alert) => {
            const metric = getMetricInfo(alert.metric_name);
            const condition = getConditionInfo(alert.comparison_operator);
            const MetricIcon = metric.icon;
